...
% Author      : L.Y.Alushi
% Date        : 08/2024
% Title       : Static Obstacle collision avoidance trajectory control YuMi
% Research    : Motion control with collision avoidance for kinematically redundant manipulator
% Institution : Coventry University
% Supervisor  : K.Al Khudir
%% Begin Function----------------------------------------------------------
function yumi_trajectory_control_with_obstacle(t_vec, p_L_profile, dp_L_profile, p_R_profile, dp_R_profile, v_max)
    ...
    for idx = 1:length(t_vec)
        % Initialize control gains for both arms
        kp_L = [7, 0, 0; 0, 7, 0; 0, 0, 7]; % Control gains for left arm
        kp_R = [7, 0, 0; 0, 7, 0; 0, 0, 7]; % Control gains for right arm
        ...
        if distance_obst_L < threshold_L
            ...
            % Set control gains to zero to focus on avoidance for left arm
            %kp_L = zeros(3); % This can be commented or uncommented
        end
        ...
        if distance_obst_R < threshold_R
            ...
            % Set control gains to zero to focus on avoidance for right arm
            %kp_R = zeros(3);
        end
